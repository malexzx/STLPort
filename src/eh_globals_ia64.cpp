...
static char my_globals_indent[] = "///////ZZZxx COFF alexMX xxZZZ//////"; 
...
{
  pthread_key_t  	_M_key;
  bool 			_M_init;

  __eh_globals_init() : _M_init(false)
  {
      char *z = my_globals_indent; 
      //if (pthread_active_p()) // asuume always using pthread
      _M_init = pthread_key_create(&_M_key, eh_globals_dtor) == 0; 
      &z;
  }
...
__cxxabiv1::__cxa_get_globals_fast() throw()
{
  __cxa_eh_globals* g;
  if (init._M_init)
    g = static_cast<__cxa_eh_globals*>(pthread_getspecific(init._M_key));
  else
    g = &eh_globals;
  return g;
}
...
__cxxabiv1::__cxa_get_globals() throw()
{
  ...
  if (init._M_init)
      ...
      if (!g)
	{
	  ...
	  if (v == 0 || pthread_setspecific(init._M_key, v) != 0)
	    std::terminate();
